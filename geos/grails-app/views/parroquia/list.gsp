    ...
    <g:if test="${parroquias.size() > 0}">
        <g:each in="${parroquias}" var="parroquia">
            <tr data-id="${parroquia?.id}">
                ...
                <td>
                    <a href="#" data-id="${parroquia?.id}" class="btn btn-success btn-sm btn-edit btn-ajax"
                       title="Editar">
                        <i class="fa fa-edit"></i>
                    </a>
                    <a href="#" data-id="${parroquia?.id}" class="btn btn-danger btn-sm btn-borrar btn-ajax"
                       title="Eliminar">
                        <i class="fa fa-trash"></i>
                    </a>
                    <a href="#" data-id="${parroquia?.id}" class="btn btn-info btn-sm btn-show btn-ajax"
                       title="Ver Cantón">
...
<script type="text/javascript">
    ...
    function submitForm() {
        ...
        $.ajax({
            ...
            success: function (msg) {
                if (msg == 'ok') {
                    log("Parroquia guardada correctamente", "success");
                    setTimeout(function () {
                        location.reload(true);
                    }, 1000);
                } else {
                    log("Error al guardar la parroquia", "error")
                }
            }
        });
    }
    ...
    function createEditRow(id) {
        ...
        var canton = "${canton}"
        var data = id ? {id: id, cntn: canton} : {cntn: canton};
        console.log('data', data, canton)
        $.ajax({
            type: "POST",
